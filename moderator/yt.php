...
$plis = '';
$bookplis = $db->get_results("SELECT id, title FROM  ".DB_PREFIX."playlists where picture not like '[%' order by title asc limit 0,200");

if($bookplis) {
$plis ='<select placeholder="Select playlist:" name="plis" class="select ">';
$plis .='<option value="0" class="opm" selected>None</option>';

foreach ($bookplis as $plo)	{
if(not_empty($plo->title)){
$plis .='<option value="'.$plo->id.'" class="opm">'.$plo->title.'</option>';
}
}
$plis .='</select>';
...
<div class="form-group form-material">
	<label class="control-label">Store videos in:</label> 	
	<div class="controls"> 	<?php echo $cats; ?>  </div> 
	<span class="help-block">Pick your category for this import. </span></div>
	
	
	<div class="form-group form-material">
	<label class="control-label">Save to playlist:</label> 	
	<div class="controls">			
		<?php echo $plis; ?>		
	</div> 
	<span class="help-block">Optionaly pick a playlist to add to. </span>
	</div>
