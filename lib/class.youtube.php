...
//var_dump($video);
    if(isset($the['plis']) || isset($_REQUEST['plis']) ) {
    $play = 0;
    if(isset($the['plis'])) { $play = intval($the['plis']); } elseif(isset($_REQUEST['plis']) ) { $play = intval($_REQUEST['plis']); }  
    if(intval($play) > 0 ) { 
        $x = $db->get_var("SELECT id from ".DB_PREFIX."videos where user_id = '".$owner."' order by id desc");
       
            if($x) {
            $db->query("INSERT INTO ".DB_PREFIX."playlist_data (`playlist`, `video_id`) VALUES (".$play.", ".$x.")");
            //$db->debug();
            //exit();
            add_activity('2', $x, $play);
            }
        }
    }
} else {
